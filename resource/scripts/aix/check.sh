...
if [ $envLang="ZH_CN.UTF-8" ]; then
   ...
   cd $updateDir
    if [ "${updateFile##*.}" = "jar" ]; then
      ...
      #按文件名排序，导出文件名
      tar tvf $updateFile | sort -d -k 8 | grep ^[^d] | awk '{for(i=8;i<NF;i++) printf $i" ";print $NF}' > updateFileList.txt
      #按文件名排序，导出文件大小，修改时间和文件名
      tar tvf $updateFile | sort -d -k 8 | grep ^[^d] | awk '{printf $4" ";for(i=8;i<NF;i++) printf $i" ";print $NF}' > tarUpdateFileList.txt
   fi
   #回到上一层目录
   cd $nextDir
   ...
   cd $bipsDir
   fileList=""
  	cat updateFileList.txt|while read line 
    do  
       fileList=$fileList" '"$line"'"
    done
   #导出bips更新文件列表
   echo $fileList | xargs ls -l |sort -d -k 8 |grep ^[^d] |awk '{printf $5" ";for(i=8;i<NF;i++) printf $i" ";print $NF}' > bipsUpdateFileList.txt
   # 统计文件差异
