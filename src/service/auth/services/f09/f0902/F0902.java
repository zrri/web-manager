package service.auth.services.f09.f0902;
 ...
 */
public class F0902 extends Service<JsonRequest, JsonResponse> {
	...
	public JsonResponse execute(JsonRequest request) throws Exception {
		...
		// 生成条件DBO
		FOX_MGR_AUTH_GROUPFUNCMAPPING_DBO wheredbo = new FOX_MGR_AUTH_GROUPFUNCMAPPING_DBO();
		// 功能组编号
		if(!StringUtilEx.isNullOrEmpty(GROUPID)){
			dbo.set_GROUPID(GROUPID);
		}
		...
		// 读取数据库
		int result = FOX_MGR_AUTH_GROUPFUNCMAPPING_DAO.update(dbo, wheredbo);
		if (result == -1) {
			// 失败
		} else {
			// 成功
