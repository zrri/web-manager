package service.auth.services.f02.f0202;
 ...
 */
public class F0202 extends Service<JsonRequest, JsonResponse> {
	...
	public JsonResponse execute(JsonRequest request) throws Exception {
		...
		// 生成条件DBO
		FOX_MGR_AUTH_TELLERINFO_DBO wheredbo = new FOX_MGR_AUTH_TELLERINFO_DBO();
		// 用户代号
		if(!StringUtilEx.isNullOrEmpty(USERID)){
			dbo.set_USERID(USERID);
		}
		...
		// 读取数据库
		int result = FOX_MGR_AUTH_TELLERINFO_DAO.update(dbo, wheredbo);
		if (result == -1) {
			// 失败
		} else {
			// 成功
