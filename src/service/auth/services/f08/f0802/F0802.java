package service.auth.services.f08.f0802;
 ...
 */
public class F0802 extends Service<JsonRequest, JsonResponse> {
	...
	public JsonResponse execute(JsonRequest request) throws Exception {
		...
		// 生成条件DBO
		FOX_MGR_AUTH_ROLEFUNCMAPPING_DBO wheredbo = new FOX_MGR_AUTH_ROLEFUNCMAPPING_DBO();
		// 角色编号
		if(!StringUtilEx.isNullOrEmpty(ROLEID)){
			dbo.set_ROLEID(ROLEID);
		}
		...
		// 读取数据库
		int result = FOX_MGR_AUTH_ROLEFUNCMAPPING_DAO.update(dbo, wheredbo);
		if (result == -1) {
			// 失败
		} else {
			// 成功
