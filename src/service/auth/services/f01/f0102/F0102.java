package service.auth.services.f01.f0102;
 ...
 */
public class F0102 extends Service<JsonRequest, JsonResponse> {
	...
	public JsonResponse execute(JsonRequest request) throws Exception {
		...
		// 生成条件DBO
		FOX_MGR_AUTH_ORGINFO_DBO wheredbo = new FOX_MGR_AUTH_ORGINFO_DBO();
		// 机构编号
		if(!StringUtilEx.isNullOrEmpty(ORGID)){
			dbo.set_ORGID(ORGID);
		}
		...
		// 读取数据库
		int result = FOX_MGR_AUTH_ORGINFO_DAO.update(dbo, wheredbo);
		if (result == -1) {
			// 失败
		} else {
			// 成功
