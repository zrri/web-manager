package service.cm.services.host.CMHostInfo02;
 ...
 */
public class CMHOSTINFO02 extends Service<JsonRequest, JsonResponse> {
	...
	public JsonResponse execute(JsonRequest request) throws Exception {
		...
		// 生成条件DBO
		FOX_MGR_CM_HOSTINFO_DBO wheredbo = new FOX_MGR_CM_HOSTINFO_DBO();
		// 主机ip
		if (!StringUtilEx.isNullOrEmpty(HOSTIP)) {
		...
		// 读取数据库
		int result = FOX_MGR_CM_HOSTINFO_DAO.update(dbo, wheredbo);
		if (result == -1) {
			// 失败
