...
define(function (require, exports) {
    ...
    function toTreeData(data) {
        ...
        function convertTreeData(chiArr) {
            nodeSort(chiArr);//对菜单节点进行排序
            // alert(JSON.stringify(chiArr))
    ...
    // //排序
    // function excSort(arr) {
    //     var i = arr.length - 1;
    //     while (i > 0) {
    //         var pos = 0;
    //         for (var j = 0; j < i; j++)
    //             if (arr[j].NODEORDER > arr[j + 1].NODEORDER) {
    //                 pos = j;
    //                 var tmp = arr[j];
    //                 arr[j] = arr[j + 1];
    //                 arr[j + 1] = tmp;
    ...
    function nodeSort(arr){
      ...
      for(var i=0;i<arr.length-1;i++)
        for(j=i+1;j<arr.length;j++){
          if(arr[i].NODEORDER>arr[j].NODEORDER){
            tmp=arr[i];
            arr[i]=arr[j];
            arr[j]=tmp;
          }
